...
/**
1. 重组器的输出是一个 byteStream,连续 当重组qi知道下一个字节 就会输出到bytestream
*/
    ...
    while (pos_iter != _unassemble_strs.end() && new_idx <= pos_iter->first) {
        ...
        if (pos_iter->first < data_end_pos) {
            ...
            // 如果是全部重叠
                ...
                _unassembled_byte_num -= pos_iter->second.size();
    ...
    if (data_size > 0) {
        ...
        if (new_idx == _next_assembled_idx) {
            ...
            if (write_byte < new_data.size()) {
                ...
                _unassembled_byte_num += data_to_store.size();
...
bool StreamReassembler::empty() const { return _unassembled_byte_num == 0; }
